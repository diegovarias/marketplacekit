...
Route::group(['prefix' => LaravelLocalization::setLocale(), 'middleware' => 'jailBanned'], function () {
    ...
    Route::get(
        'email-verification',
        'Auth\EmailVerificationController@sendEmailVerification'
    )->name('email-verification.send');

    Route::get(
        'email-verification/error',
        'Auth\EmailVerificationController@getVerificationError'
    )->name('email-verification.error');

    Route::get(
        'email-verification/check/{token}',
        'Auth\EmailVerificationController@getVerification'
    )
    ...
    Route::get('/profile/{user}/follow', 'ProfileController@follow')->name('profile.follow');
    ...
    Route::group(['prefix' => 'listing'], function () {
        ...
        Route::get(
            '/{listing}/{slug}/spotlight',
            'ListingController@spotlight'
        )->middleware('auth.ajax')->name('listing.spotlight');
        
        Route::get(
            '/{listing}/{slug}/verify',
            'ListingController@verify'
        )->middleware('auth.ajax')->name('listing.verify');
    ...
    Route::group(['middleware' => ['auth', 'isVerified']], function () {
        Route::resource('inbox', 'InboxController')->middleware('talk');
